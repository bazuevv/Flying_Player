...
4. Удаляет старый webview/claude-custom.js (если остался от предыдущей
   попытки грузить JS отдельным файлом — там CSP его блокировал).
5. Чистит устаревший блок `/* claude-green-timestamp */` из
   `webview/index.css` (раньше CSS-правило жило прямо там).
...
def _strip_marker_block(path: str) -> bool:
    """Вырезать блок `/* claude-green-timestamp */ … */` из файла."""
    try:
        content = _read(path)
    except OSError:
        return False
...
def main() -> int:
    ...
    for ext_dir in glob.glob(EXT_GLOB):
        ...
        # 2. Удаляем webview/claude-custom.js, если он остался от прошлой версии
        #    хука (когда мы пытались грузить JS отдельным файлом — CSP блокировал)
        stale_js = os.path.join(webview_dir, "claude-custom.js")
        if os.path.isfile(stale_js):
            try:
                os.remove(stale_js)
            except OSError:
                pass

        # 3. Очищаем устаревший блок из index.css (если остался от прошлой версии)
        index_css = os.path.join(webview_dir, "index.css")
        if os.path.isfile(index_css):
            _strip_marker_block(index_css)

        # 4. Вставляем/обновляем bootstrap-блок в index.js
        index_js = os.path.join(webview_dir, "index.js")
        if os.path.isfile(index_js):
            _upsert_marker_block(index_js, bootstrap)

    # 5. Если в конфиге есть проблемы — кладём предупреждение в
